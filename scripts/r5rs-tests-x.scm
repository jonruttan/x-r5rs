; r5rs-tests-x.scm -- R5RS compliance tests adapted for x-lang
; Based on r5rs-tests.scm
;
; Skipped tests:
;   - call-with-current-continuation (not implemented)
;   - dynamic-wind (requires call/cc)
;   - set-cdr! circular list (list? would infinite-loop)
;   - custom ellipsis identifier ::: (not implemented)
;   - (not '()) / (not (list)) / (boolean? '()) -- nil=false in x-lang
;
; Known differences:
;   - x-lang: nil = () = #f, so (not '()) => #t (not #f as R5RS expects)
;   - Nested quasiquote may not fully work
...
;; Nested quasiquote -- may not work in x-lang
...
;; SKIP: x-lang nil=false, so (not '()) => #t, not #f
;; (test #f (not '()))
;; (test #f (not (list)))
;; (test #f (not '()))

;; SKIP: x-lang nil=false, so (boolean? '()) => #t, not #f
;; (test #f (boolean? 0))
;; (test #f (boolean? '()))
...
;; SKIP: set-cdr! circular list would infinite-loop list?
...
;; SKIP: x-lang nil=false, symbol? on '() depends on nil representation
;; (test #f (symbol? '()))
...
;; SKIP: call-with-current-continuation not implemented
...
;; Cond pitfall tests -- may or may not work depending on hygiene
...
;; SKIP: custom ellipsis identifier ::: not implemented
;; (test 2 (let-syntax
;;             ((foo (syntax-rules ::: ()
;;                     ((foo ... args :::)
;;                      (args ::: ...)))))
...
;; SKIP: define inside let-syntax/letrec-syntax doesn't persist (eval saves/restores env)
;; (test 'ok (let () (let-syntax () (define internal-def 'ok)) internal-def))
;; (test 'ok (let () (letrec-syntax () (define internal-def 'ok)) internal-def))
...
;; SKIP: dynamic-wind requires call/cc
;; (test '(a b c) ...)
